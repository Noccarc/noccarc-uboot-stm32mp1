...
static int otm8009a_init_sequence(struct udevice *dev)
{
	log_info("[panel] Entering init sequence\n");
	struct mipi_dsi_panel_plat *plat = dev_get_platdata(dev);
	struct udevice *dev1;
	int ret, i;
	u8 val;
	...
	// Get SN65DSI83 device on I2C3
	ret = i2c_get_chip_for_busnum(3, 0x2c, 1, &dev1);
	if (ret) return ret;
	log_info("[panel] Programming bridge registers\n");
	dm_i2c_reg_write(dev1, REG_RC_PLL_EN, 0x00);
	mdelay(1);
	dm_i2c_reg_write(dev1, REG_RC_LVDS_PLL, 0x05);
	dm_i2c_reg_write(dev1, REG_DSI_CLK, 0x48);
	dm_i2c_reg_write(dev1, REG_RC_DSI_CLK, 0x28);
	dm_i2c_reg_write(dev1, REG_RC_PLL_EN, 0x00);
	dm_i2c_reg_write(dev1, REG_DSI_LANE, 0x30);
	dm_i2c_reg_write(dev1, REG_DSI_EQ, 0x00);
	dm_i2c_reg_write(dev1, REG_LVDS_FMT, 0x78);
	dm_i2c_reg_write(dev1, REG_LVDS_VCOM, 0x00);
	dm_i2c_reg_write(dev1, REG_LVDS_LANE, 0x00);
	dm_i2c_reg_write(dev1, REG_LVDS_CM, 0x00);
	
	// Resolution
	dm_i2c_reg_write(dev1, REG_VID_CHA_ACTIVE_LINE_LENGTH_LOW, hactive & 0xff);
	dm_i2c_reg_write(dev1, REG_VID_CHA_ACTIVE_LINE_LENGTH_HIGH, hactive >> 8);
	dm_i2c_reg_write(dev1, REG_VID_CHA_VERTICAL_DISPLAY_SIZE_LOW, vactive & 0xff);
	dm_i2c_reg_write(dev1, REG_VID_CHA_VERTICAL_DISPLAY_SIZE_HIGH, vactive >> 8);

	// Porch and sync
	dm_i2c_reg_write(dev1, REG_VID_CHA_HSYNC_PULSE_WIDTH_LOW, hsw & 0xff);
	dm_i2c_reg_write(dev1, REG_VID_CHA_HSYNC_PULSE_WIDTH_HIGH, hsw >> 8);
	dm_i2c_reg_write(dev1, REG_VID_CHA_VSYNC_PULSE_WIDTH_LOW, vsw & 0xff);
	dm_i2c_reg_write(dev1, REG_VID_CHA_VSYNC_PULSE_WIDTH_HIGH, vsw >> 8);
	dm_i2c_reg_write(dev1, REG_VID_CHA_HORIZONTAL_BACK_PORCH, hbp);
	dm_i2c_reg_write(dev1, REG_VID_CHA_VERTICAL_BACK_PORCH, vbp);
	dm_i2c_reg_write(dev1, REG_VID_CHA_HORIZONTAL_FRONT_PORCH, hfp);
	dm_i2c_reg_write(dev1, REG_VID_CHA_VERTICAL_FRONT_PORCH, vfp);
	...
	log_info("[panel] Enabling PLL and waiting for lock\n");
	...
	log_info("[panel] PLL locked. Sending reset\n");
...
static int otm8009a_panel_enable_backlight(struct udevice *dev)
{
	...
	if (ret < 0)
		return ret;
	// Step 1: Init bridge (SN65DSI83) and wait for PLL lock
...
//	mdelay(200);
	...
	// Step 2: Wait ≥500ms after LVDS active before turning on backlight
	log_info("[panel] Waiting 500ms for panel stabilization\n");
	mdelay(500); // T3

	log_info("[panel] Enabling power and PWM GPIOs\n");
	...
	log_info("[panel] Backlight enabled\n");
...
{
	log_info("[panel] Parsing GPIOs and regulators\n");
	struct otm8009a_panel_priv *priv = dev_get_priv(dev);
	int ret;
	ofnode node;
