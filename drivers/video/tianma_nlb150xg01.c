...
//	mdelay(200);
	...
	// Step 3: Enable GPIOs (as per datasheet: VENH ≥ 3V, PWM ≥ 3V)
	dm_gpio_set_value(&priv->enable, true);         // SN65DSI83 EN or LVDS_PWR_EN
	dm_gpio_set_value(&priv->backlight_en, true);   // LED_EN
	dm_gpio_set_value(&priv->backlight_pwm, true);  // LED_PWM

	log_info("driver: Backlight enabled after 500ms delay\n");
